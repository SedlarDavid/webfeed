...
void main() {
  group('XML namespace handling tests', () {
    test('Find elements with namespace prefix', () {
      ...
      final allContentElements = findAllElementsWithNamespace(document, 'encoded');
      expect(allContentElements.length, 2);
      
      // Test finding elements with namespace prefix (direct children only)
      final directContentElements = findAllElementsWithNamespace(feedElement, 'encoded')
      ...
      final entryContentElement = findElementWithNamespace(entryElement, 'encoded');
    ...
    test('Find elements with alternative names', () {
      ...
      final authorElement = findElementWithAlternatives(
          channelElement, ['author', 'creator']);
    ...
    test('Get attribute with namespace', () {
      ...
      final allHrefs = linkElement.attributes
          .where((attr) => attr.name.local == 'href' || 
    ...
    test('Parse boolean values with fallback', () {
      ...
      expect(findElementWithNamespace(itemElement, 'explicit')?.value, 'yes');
      ...
      expect(findElementWithNamespace(itemElement, 'nonBoolean')?.value, 'something');
