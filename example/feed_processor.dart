...
Future<void> processFeedList(IOClient client, List<String> urls) async {
  for (var i = 0; i < urls.length; i++) {
    ...
    try {
      ...
      final response = await client.get(Uri.parse(secureUrl))
          ...
          case FeedType.unknown:
            ...
            try {
              ...
              try {
                ...
                print('  Failed to parse feed as RSS or Atom');
...
Future<void> _processRssFeed(RssFeed feed) async {
  ...
  print('  Description: ${feed.description?.substring(0, 
      feed.description != null && feed.description!.length > 50 
          ? 50 : feed.description?.length ?? 0)}...');
